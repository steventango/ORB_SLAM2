...
void ViewerRR::SetImagePose(const cv::Mat &im, const cv::Mat &Tcw,
                            const int &status,
...
void ViewerRR::GetImagePose(cv::Mat &im, cv::Mat &Tcw, int &status,
...
void ViewerRR::LoadCameraPose(const cv::Mat &Tcw) {
    if (!Tcw.empty()) {
...
void ViewerRR::PrintStatus(const int &status, const bool &bLocMode,
                           cv::Mat &im) {
    if (!bLocMode) {
        switch (status) {
            case 1: {
...
void ViewerRR::AddTextToImage(const string &s, cv::Mat &im, const int r,
...
void ViewerRR::DrawImageTexture(pangolin::GlTexture &imageTexture,
                                cv::Mat &im) {
    if (!im.empty()) {
...
void ViewerRR::DrawCube(const float &size, const float x, const float y,
                        const float z) {
    pangolin::OpenGlMatrix M =
...
void ViewerRR::DrawPlane(Plane *pPlane, int ndivs, float ndivsize) {
    glPushMatrix();
    pPlane->glTpw.Multiply();
...
void ViewerRR::DrawPlane(int ndivs, float ndivsize) {
...
void ViewerRR::DrawTrackedPoints(const std::vector<cv::KeyPoint> &vKeys,
...
Plane *ViewerRR::DetectPlane(const cv::Mat Tcw,
                             const std::vector<MapPoint *> &vMPs,
