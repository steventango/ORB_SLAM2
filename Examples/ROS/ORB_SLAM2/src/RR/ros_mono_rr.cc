...
int main(int argc, char** argv) {
    ...
    if (argc != 3) {
        cerr << endl
             << "Usage: rosrun ORB_SLAM2 Mono path_to_vocabulary "
                "path_to_settings"
             << endl;
        ros::shutdown();
        return 1;
    ...
    viewerRR.SetCameraCalibration(fx, fy, cx, cy);
    ...
    thread tViewer = thread(&ORB_SLAM2::ViewerRR::Run, &viewerRR);
...
void ImageGrabber::GrabImage(const sensor_msgs::ImageConstPtr& msg) {
    ...
    if (bRGB)
        viewerRR.SetImagePose(imu, Tcw, state, vKeys, vMPs);
    else {
